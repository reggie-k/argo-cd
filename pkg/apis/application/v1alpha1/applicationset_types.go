...
type ApplicationSetApplicationStatus struct {
  ...
  // Progressive Sync fields
	...
	ProgressiveSyncLastTransitionTime *metav1.Time `json:"lastTransitionTime,omitempty" protobuf:"bytes,5,opt,name=lastTransitionTime"`
	// ProgressiveSyncMessage contains human-readable message indicating details about the status of the progressive sync
	ProgressiveSyncMessage string `json:"message" protobuf:"bytes,6,opt,name=message"`
  ...
  // this is NOT the same as the Application's status
	ProgressiveSyncStatus string `json:"status" protobuf:"bytes,7,opt,name=status"`
	// ProgressiveSyncStep tracks which step the Applications should be updated in
	ProgressiveSyncStep string `json:"step" protobuf:"bytes,8,opt,name=step"`
}
