...
export async function deleteApplication(isApp: boolean, appName: string, appNamespace: string, apis: ContextApis): Promise<boolean> {
    let confirmed = false;
    ...
    await apis.popup.prompt(
        ...
        ),
        {
            ...
            submit: async (vals, _, close) => {
                try {
                    await services.applications.delete(isApp, appName, appNamespace, vals.propagationPolicy);
                    confirmed = true;
                    close();
                } catch (e) {
        ...
        ),
        {
            ...
            submit: async (vals, _, close) => {
                ...
                try {
                    ...
                    if (appChanged) {
                        appChanged.next(await services.applications.get(isApp(application), application.metadata.name, application.metadata.namespace));
                    }
                    close();
                } catch (e) {
...
function getActionItems(
    ...
    const links = services.applications
        .getResourceLinks(isApp(application), application.metadata.name, application.metadata.namespace, resource)
        .then(data => {
...
export const HealthStatusIcon = ({ state, noSpin }: { state: appModels.HealthStatus; noSpin?: boolean }) => {
    ...
    if (state === null || state?.status === undefined) {
        return <i qe-id='utils-health-status-title' className={'fa ' + icon} style={{ color }} />;
    }
...
export function isInvokedFromApps(isApp : boolean): boolean {
    // console.log("*** ctx.history.location.pathname *** " + ctx.history.location.pathname)
    const isInvokedFromApp = isApp //ctx.history.location.pathname.includes("applicationsets") ? true : false
...
export function getRootPath(isApp: boolean): string {
    return isApp ? '/applications' : '/applicationsets'
}
