...
import { isAppSetFilterResult } from '../applications-list/applications-filter';
...
export class ApplicationDetails extends React.Component<RouteComponentProps<{ appnamespace: string; name: string }>, ApplicationDetailsState> {
                    ...
                    <DataLoader
                        ...
                        load={name =>
                            combineLatest([this.loadAppInfo(name, this.appNamespace), services.viewPreferences.getPreferences(), q]).pipe(
                                map(items => {
                                    console.log("*** in combine latest 1")
                        ...
                        {({ application, tree, pref }: { application: appModels.AbstractApplication; tree: appModels.ApplicationTree; pref: AppDetailsPreferences }) => {
                            ...
                            const resourceNodes = (): any[] => {
                                ...
                                if (!AppUtils.isAppSet(application)) {
                            ...
                            return (
                                <div className={`application-details ${this.props.match.params.name}`}>
                                    <Page
                                        ...
                                        toolbar={{
                                            breadcrumbs: [
                                                !AppUtils.isAppSet(application) ? { title: 'Applications', path: '/applications' } : { title: 'Settings', path: '/settings' },
    ...
    private loadAppInfo(name: string, appNamespace: string): Observable<{ application: appModels.Application; tree: appModels.ApplicationTree }> {
        // debugger;
        console.log("loadAppInfo1********************" + name + "************")
        return from(services.applications.get(name, appNamespace))
            .pipe(
                mergeMap(app => {
                    const fallbackTree = {
                        nodes: !AppUtils.isAppSet(app) ? (app as models.Application).status.resources.map((res: any) => ({ ...res, parentRefs: [], info: [], resourceVersion: '', uid: '' })) : [],
                        orphanedNodes: [],
                        hosts: []
                    } as appModels.ApplicationTree;
                    console.log("loadAppInfo2********************" + name + "************")
