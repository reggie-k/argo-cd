...
export const ResourceDetails = (props: ResourceDetailsProps) => {
    ...
    const getApplicationTabs = () => {
            ...
            content: (
                <YamlEditor
                    ...
                    onSave={async patch => {
                        ...
                        return services.applications.updateSpec(isApp(application), application.metadata.name, application.metadata.namespace, jsonMergePatch.apply(spec, JSON.parse(patch)));
                    }}
                />
            )
        })
            ...
            tabs.push({
                ...
                content: (
                    <DataLoader
                        key='diff'
                        load={async () =>
                            await services.applications.managedResources(isApp(application), application.metadata.name, application.metadata.namespace, {
        ...
        tabs.push({
            title: 'EVENTS',
            key: 'event',
            content: <ApplicationResourceEvents isApp={isApp(application)} applicationName={application.metadata.name} applicationNamespace={application.metadata.namespace} />
        });
        ...
        <div style={{ width: '100%', height: '100%' }}>
            {selectedNode && (
                <DataLoader
                    ...
                    load={async () => {
                        const managedResources = await services.applications.managedResources(isApp(application), application.metadata.name, application.metadata.namespace, {
                            id: {
                        ...
                        const liveState = await services.applications.getResource(isApp(application), application.metadata.name, application.metadata.namespace, resQuery).catch(() => null);
                        const events =
                            (liveState &&
                                (await services.applications.resourceEvents(isApp(application), application.metadata.name, application.metadata.namespace, {
                        ...
                        if (selectedNode.kind === 'Pod') {
                            ...
                            if (childPod) {
                                podState = await services.applications.getResource(isApp(application), application.metadata.name, application.metadata.namespace, childPod).catch(() => null);
                            }
                        }
                        ...
                        const links = await services.applications.getResourceLinks(isApp(application), application.metadata.name, application.metadata.namespace, selectedNode).catch(() => null);
