...
export const ApplicationsSummary = ({ isAppSet, applications }: { isAppSet: boolean, applications: models.AbstractApplication[] }) => {
    const sync = new Map<string, number>();
    const health = new Map<string, number>();

    console.log("*************** ApplicationsSummary isAppSet = " + isAppSet)
    ...
    const attributes = isInvokedFromApps(!isAppSet) ? ([{
        title: 'APPLICATIONS',
    ...
    const charts = isInvokedFromApps(!isAppSet) ? ([
        {
            title: 'Sync',
                                                ...
                                                <ul>
                                                    {Array.from(chart.legend.keys()).map(key => (
                                                        <li style={{ listStyle: 'none', whiteSpace: 'nowrap' }} key={key}>
                                                            {isInvokedFromApps(!isAppSet) && chart.title === 'Health' && <HealthStatusIcon state={{ status: key as HealthStatusCode, message: '' }} noSpin={true} />}
