...
export const ApplicationsTable = (props: {
    ...
    useKeybinding({
        ...
        action: () => {
            if (selectedApp > -1) {
                const isApp = props.applications[selectedApp].kind === 'Application';
                ctxh.navigation.goto(`${AppUtils.getRootPath(isApp)}/${props.applications[selectedApp].metadata.name}`);
                return true;
            }
            return false;
        }
            ...
            {ctx => (
                ...
                applications-list__entry applications-list__entry--health-${isApp(app) ? (app as models.Application).status.health.status : getAppSetHealthStatus((app as ApplicationSet).status)} ${selectedApp === i ? 'applications-tiles__selected' : ''}`}>
                                        <div
                                            ...
                                            onClick={e => ctx.navigation.goto(`${AppUtils.getRootPath(isApp(app))}/${app.metadata.namespace}/${app.metadata.name}`, {}, { event: e })}>
