...
export const ApplicationsTable = (props: {
    ...
    useKeybinding({
        ...
        action: () => {
            if (selectedApp > -1) {
                ctxh.navigation.goto(`${AppUtils.getRootPath(props.applications[selectedApp])}${props.applications[selectedApp].metadata.name}`);
                return true;
            }
            return false;
        }
    ...
    return (
        <Consumer>
            {ctx => (
                ...
                applications-list__entry applications-list__entry--health-${AppUtils.isAppSet(app) ? (app as models.ApplicationSet).status.conditions[0].status : (app as models.Application).status.health.status} ${selectedApp === i ? 'applications-tiles__selected' : ''}`}>
                                        <div
                                            className={`row applications-list__table-row`}
                                            onClick={e => ctx.navigation.goto(`${AppUtils.getRootPath(app)}${app.metadata.namespace}/${app.metadata.name}`, {}, { event: e })}>
                                                ...
                                                <div className='row'>
                                                    <div className=' columns small-2'>
                                                        <div>
                                                            ...
                                                            {!AppUtils.isAppSet(app) && <ApplicationURLs urls={(app as models.Application).status.summary.externalURLs} />}
                                                        </div>
                                                    </div>
                                                    {!AppUtils.isAppSet(app) && <div className='show-for-xxlarge columns small-4'>Project:</div>}
                                                    {!AppUtils.isAppSet(app) && <div className='columns small-12 xxlarge-6'>{(app as models.Application).spec.project}</div>}
                                                </div>
                                                <div className='row'>
                                                    <div className=' columns small-2' />
                                            ...
                                            {!AppUtils.isAppSet(app) && (<div className='columns small-6'>
                                                <div className='row'>
                                            ...
                                            <div className='columns small-2'>
                                                {!AppUtils.isAppSet(app) && <AppUtils.HealthStatusIcon state={(app as models.Application).status.health} />} {!AppUtils.isAppSet(app) && <span>{(app as models.Application).status.health.status}</span>} <br />
                                                {AppUtils.isAppSet(app) && <AppUtils.AppSetHealthStatusIcon state={(app as models.ApplicationSet).status} />} {AppUtils.isAppSet(app) && <span>{(app as models.ApplicationSet).status.conditions[0].status}</span>}F <br />
                                                {!AppUtils.isAppSet(app) && <AppUtils.ComparisonStatusIcon status={(app as models.Application).status.sync.status} />}
                                                {!AppUtils.isAppSet(app) && <span>{(app as models.Application).status.sync.status}</span>} {!AppUtils.isAppSet(app) && <OperationState app={(app as models.Application)} quiet={true} />}
                                                <DropDownMenu
                                                    anchor={() => (
