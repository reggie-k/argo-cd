...
function getQuery(isAppSet: boolean, projects: string[], options?: QueryOptions): any {
    if (!isAppSet) {
...
export class ApplicationsService {
    public list(isAppSet: boolean, projects: string[], options?: QueryOptions): Promise<models.AbstractApplicationList> {
        return requests
            .get(getRootPath(!isAppSet))
            .query(getQuery(isAppSet, projects, options))
            .then(res => {
                if (!isAppSet) {
            ...
            .then(list => {
                list.items = (list.items || []).map(app => this.parseAppFields(!isAppSet, app));
                
                return list;
            });
    }

    public get(isApp: boolean, name: string, appNamespace: string, refresh?: 'normal' | 'hard'): Promise<models.AbstractApplication> {
        ...
        return requests
            .get(`${getRootPath(isApp)}/${name}`)
            .query(query)
            .then(res => this.parseAppFields(isApp, res.body));
    }
    ...
    public resourceTree(isApp: boolean, name: string, appNamespace: string): Promise<models.ApplicationTree> {
        return requests
            .get(`${getRootPath(isApp)}/${name}/resource-tree`)
            .query({ appNamespace })
            .then(res => res.body as models.ApplicationTree);
    }

    public watchResourceTree(isApp: boolean, name: string, appNamespace: string): Observable<models.ApplicationTree> {
        return requests
            .loadEventSource(`/stream${getRootPath(isApp)}/${name}/resource-tree?appNamespace=${appNamespace}`)
            .pipe(map(data => JSON.parse(data).result as models.ApplicationTree));
    }

    public managedResources(isApp: boolean, name: string, appNamespace: string, options: { id?: models.ResourceID; fields?: string[] } = {}): Promise<models.ResourceDiff[]> {
        return requests
            .get(`${getRootPath(isApp)}/${name}/managed-resources`)
    ...
    public getManifest(isApp: boolean, name: string, appNamespace: string, revision: string): Promise<models.ManifestResponse> {
        return requests
            .get(`${getRootPath(isApp)}/${name}/manifests`)
            .query({ name, revision, appNamespace })
            .then(res => res.body as models.ManifestResponse);
    }

    public updateSpec(isApp: boolean,appName: string, appNamespace: string, spec: models.ApplicationSpec): Promise<models.ApplicationSpec> {
        return requests
            .put(`${getRootPath(isApp)}/${appName}/spec`)
            .query({ appNamespace })
            .send(spec)
    ...
    public update(app: models.AbstractApplication, query: { validate?: boolean } = {}): Promise<models.AbstractApplication> {
        return requests
            .put(`${getRootPath(app.kind === "Application")}/${app.metadata.name}`)
            .query(query)
            .send(app.kind == "application" ? app as models.Application : app as models.ApplicationSet)
            .then(res => this.parseAppFields(app.kind == "application", res.body));
    }
    ...
    public create(app: models.AbstractApplication): Promise<models.AbstractApplication> {
        ...
        return requests
            .post(getRootPath(app.kind === "Application"))
            .send(app.kind === "application" ? app as models.Application : app as models.ApplicationSet)
            .then(res => this.parseAppFields(app.kind == "application", res.body));
    }

    public delete(isApp: boolean, name: string, appNamespace: string, propagationPolicy: string): Promise<boolean> {
        let cascade = true;
    ...
    public watch(isAppSet: boolean, query?: { name?: string; resourceVersion?: string; projects?: string[]; appNamespace?: string }, options?: QueryOptions): Observable<models.ApplicationWatchEvent> {
        ...
        if (options) {
            ...
            if (isInvokedFromApps(!isAppSet)) {
                query?.projects?.forEach(project => search.append('projects', project));
            }
        }
        const searchStr = search.toString();
        const url = `/stream${getRootPath(!isAppSet)}${(searchStr && '?' + searchStr) || ''}`;
        return requests
            .loadEventSource(url)
            .pipe(repeat())
            .pipe(retry())
            ...
            .pipe(
                map(watchEvent => {
                    watchEvent.application = this.parseAppFields(!isAppSet, watchEvent.application);
                    return watchEvent;
                })
            );
    }
    ...
    public getResource(isApp: boolean, name: string, appNamespace: string, resource: models.ResourceNode): Promise<models.State> {
        return requests
            .get(`${getRootPath(isApp)}/${name}/resource`)
            .query({
                name: resource.name,
                appNamespace,
    ...
    public events(isApp: boolean, applicationName: string, appNamespace: string): Promise<models.Event[]> {
        return requests
            .get(`${getRootPath(isApp)}/${applicationName}/events`)
            .query({ appNamespace })
            .send()
    ...
    public getLinks(isApp: boolean, applicationName: string, namespace: string): Promise<models.LinksResponse> {
        return requests
            .get(`${getRootPath(isApp)}/${applicationName}/links`)
            .query({ namespace })
            .send()
            .then(res => res.body as models.LinksResponse);
    }

    public getResourceLinks(isApp: boolean, applicationName: string, appNamespace: string, resource: models.ResourceNode): Promise<models.LinksResponse> {
        return requests
            .get(`${getRootPath(isApp)}/${applicationName}/resource/links`)
            .query({
                name: resource.name,
                appNamespace,
                namespace: resource.namespace,
    ...
    private parseAppFields(isApp: boolean, data: any): models.AbstractApplication {
        if (isInvokedFromApps(isApp)) {
            data = deepMerge(
                {
