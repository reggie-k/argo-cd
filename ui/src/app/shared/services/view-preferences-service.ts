...
const APPSET_VIEW_PREFERENCES_KEY = 'view_app_set_preferences';
...
const APPSET_DEFAULT_PREFERENCES: AppSetViewPreferences = {
    version: 1,
    appDetails: {
        view: 'tree',
        hideFilters: false,
...
export class ViewAppSetPreferencesService extends AbstractViewPreferencesService {
    protected preferencesSubj: BehaviorSubject<AppSetViewPreferences>;

    public updatePreferences(change: Partial<AppSetViewPreferences>) {
        const nextPref = Object.assign({}, this.preferencesSubj.getValue(), change, {version: minVer});
        window.localStorage.setItem(APPSET_VIEW_PREFERENCES_KEY, JSON.stringify(nextPref));
        this.preferencesSubj.next(nextPref);
    }

    protected loadPreferences(): AppSetViewPreferences {
        let preferences: AppSetViewPreferences;
        const preferencesStr = window.localStorage.getItem(APPSET_VIEW_PREFERENCES_KEY);
        if (preferencesStr) {
            try {
                preferences = JSON.parse(preferencesStr);
            } catch (e) {
                preferences = APPSET_DEFAULT_PREFERENCES;
            }
            if (!preferences.version || preferences.version < minVer) {
                preferences = APPSET_DEFAULT_PREFERENCES;
            }
        ...
        return deepMerge(APPSET_DEFAULT_PREFERENCES, preferences);
    }
}
